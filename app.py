...
st.markdown("### Veta Isabel - Línea de Tiempo Operativa")
    ...
    try:
        ...
        if fechas_indices:
            ...
            if datos:
                ...
                # --- 1. HITOS (Líneas Rojas Segmentadas: 8AM, 8PM, 8AM next day) ---
                for h in [8, 20, 32]:
                    pos = fecha_ref + timedelta(hours=h)
                    ax.axvline(pos, color="red", linestyle="--", linewidth=2, zorder=20, alpha=0.8)
                    label = "8 AM" if (h==8 or h==32) else "8 PM"
                    ax.text(pos, 19, label, color="red", fontsize=10, ha='center', fontweight='bold')

                # --- 2. ZONAS DE ALMUERZO / CENA / CHARLAS ---
                zonas = [
                ...
                for z_ini, z_fin, z_col, z_lbl in zonas:
                    ...
                    ax.text(fecha_ref + timedelta(hours=(z_ini+z_fin)/2), -18.5, z_lbl, color="white", fontsize=8, ha='center', alpha=0.6)
